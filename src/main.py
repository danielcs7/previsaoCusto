...
df_filtrado = pd.read_csv('//Volumes//MACBACKUP//BootCampEngDados//CientistaDados//projectPreveCusto//storage//dadosInterMed.csv')
...
df_filtrado_prophet = df_filtrado[['Data', 'CustoMensal']].copy()
...
def plotar_serie(df, medias_moveis, data_inicio, data_fim):
    ...
    fig.add_trace(go.Scatter(x=df_filtrado['Data'], y=df_filtrado['CustoMensal'], mode='lines+markers', name='Série Histórica'))

    # Adicionar médias móveis selecionadas
    for media in medias_moveis:
        if media == 'Média Móvel de 7 dias':
            df_filtrado['media_movel_7'] = df_filtrado['CustoMensal'].rolling(window=7).mean()
            ...
            df_filtrado['media_movel_15'] = df_filtrado['CustoMensal'].rolling(window=15).mean()
            ...
            df_filtrado['media_movel_30'] = df_filtrado['CustoMensal'].rolling(window=30).mean()
            fig.add_trace(go.Scatter(x=df_filtrado['Data'], y=df_filtrado['media_movel_30'], mode='lines', name='Média Móvel de 30 dias'))

    # Configurar layout do gráfico
    fig.update_layout(title='Plano Intermediário', xaxis_title='Data', yaxis_title='CustoMensal')
    return fig

def main():
    st.set_page_config(layout="wide")
    st.title('Painel de Séries Temporais')
    st.markdown('Planos - Uniclass')
